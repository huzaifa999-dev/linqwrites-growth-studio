...
import accountManagerAsset from "@/assets/account-manager.png.asset.json";
        ...
        <div className="relative mt-24 flex flex-col items-center text-center">
          ...
          >
            <motion.span
              ...
              className="absolute inset-6 rounded-full border border-accent-warm/40"
            />
            <motion.span
              aria-hidden
              animate={{ scale: [1, 1.35, 1], opacity: [0.3, 0, 0.3] }}
              transition={{ duration: 2.6, repeat: Infinity, ease: "easeOut", delay: 0.6 }}
              className="absolute inset-6 rounded-full border border-accent-warm/25"
            />
            <motion.img
              src={beeAsset.url}
              alt="Bee — tap to imagine anything"
              draggable={false}
              animate={
                flying
                  ? {
          ...
          <p className="mt-8 text-[0.7rem] uppercase tracking-[0.35em] text-ink-soft">Tap the bee</p>
